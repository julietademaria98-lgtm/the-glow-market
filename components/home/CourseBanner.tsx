...
export default function CourseBanner() {
  return (
    ...
    >
      {/* Texto — ocupa el 50% izquierdo, centrado dentro */}
      <motion.div
        initial={{ opacity: 0, x: -30 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
        className="relative z-10 w-1/2 flex justify-center items-center h-full"
      >
        <div className="flex flex-col" style={{ maxWidth: '380px', padding: '40px 20px' }}>
          <span
      ...
      {/* Imagen PNG — 50% derecho, apoyada abajo */}
      <motion.div
        initial={{ opacity: 0, x: 30 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
